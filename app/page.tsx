                ...
                <div className="relative group h-full">
                  ...
                  <div className="transition-all duration-200 h-full" style={{aspectRatio: '1180/1756'}}>
                    ...
                    >
                      ...
                      <div className="carousel-bottom-section pl-4 pr-2 pb-4 w-full flex flex-col gap-1" style={{ height: '25%', fontFamily: 'Poppins'}}>
                        {/* Item name and points row */}
                        <div className="flex justify-between items-center w-full">
                          <span className="carousel-item-name font-bold text-white" style={{ fontSize: '26px', width: '55%', lineHeight: '26px'}}>{item.name}</span>
                          <div className="carousel-points-container flex flex-col items-end gap-1 justify-start" style={{ alignSelf: 'flex-start', width: '45%'}}>
                            <div className="carousel-points-wrapper flex pt-2 items-start gap-1">
                              ...
                              <span className="carousel-points-text font-bold text-white p-0" style={{ fontSize: '17px', lineHeight: '17px' }}>{item.points.toLocaleString()} Pts</span>
                            </div>
                          </div>
                        </div>
                        
                        {/* Item model/description */}
                        <div className="carousel-item-model text-sm text-gray-300">
                          Item Name Model #1
                        </div>
                      </div>
                    </div>
        ...
        <div className="lg:hidden relative z-50">
          ...
          <div className="mobile-filter-dropdown absolute left-0 right-0 mx-4 mt-2 bg-gray-800 rounded-lg p-4 shadow-2xl max-h-[70vh] overflow-y-auto">
            ...
            <div className="mobile-filter-section-wrapper mb-4">
              <h4 className="mobile-filter-heading text-xs font-semibold text-white uppercase tracking-wider mb-2">Category</h4>
              <div className="space-y-1">
                {categories.map(cat => (
                  <label key={cat} className={`mobile-filter-label flex items-center gap-2 cursor-pointer px-2 py-1.5 rounded-lg transition-all ${categoryFilter.includes(cat) ? 'bg-yellow-500/10 border border-yellow-500/30' : 'hover:bg-gray-700/50'}`}>
                    <input type="checkbox" checked={categoryFilter.includes(cat)} onChange={() => toggleCategoryFilter(cat)} className="mobile-filter-checkbox rounded text-yellow-500" />
                    <span className={`mobile-filter-text text-xs ${categoryFilter.includes(cat) ? 'text-yellow-400 font-medium' : 'text-gray-300'}`}>{cat}</span>
                  </label>
                ))}
              </div>
            </div>
            {/* Mobile Tier Filter */}
            <div className="mobile-filter-section-wrapper mb-4">
              <h4 className="mobile-filter-heading text-xs font-semibold text-white uppercase tracking-wider mb-2">Prestige Tier</h4>
              <div className="space-y-1">
                {tiers.map(tier => (
        ...
        <main className="main-content flex-1 flex flex-col items-center px-3 sm:px-4 md:px-8 py-4 overflow-visible">
          ...
          {paginatedRewards.map((item) => {
            ...
            <div key={item.id} className={`reward-card-wrapper relative hover:scale-105 transition-all duration-200 h-full group ${(isLowStock || isLastOne) ? 'z-10 hover:z-30' : 'z-0 hover:z-30'} ${isOutOfStock ? 'cursor-not-allowed' : ''}`} style={{ overflow: 'visible' }}>
              ...
              <div className="reward-card-model font-normal text-xs text-left w-full text-white drop-shadow-lg" style={{ lineHeight: '10px' }}>Item Name Model</div>
          ...
          <div className="claims-checker-backdrop fixed inset-0 z-[10000] flex items-center justify-center bg-black bg-opacity-80 p-4 animate-fadeIn" onClick={() => {
            ...
            <div className="claims-checker-modal shadow-2xl p-8 max-w-lg w-full animate-scaleIn relative overflow-hidden" style={{ 
              ...
              <div className="text-center mb-6">
                <h2 className="claims-checker-title text-3xl font-extrabold mb-2" style={{ background: 'linear-gradient(135deg, #FF7901, #FFA323)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>Claims Checker</h2>
              ...
              <div className="popup-form px-4 md:px-20 pb-4">
                ...
                <form className="popup-claim-form space-y-2" onSubmit={async (e) => { 
                  ...
                  const claimData = {
                    ...
                    phoneNumber: phoneNumber ? `+639${phoneNumber}` : null,
                    deliveryAddress: formData.get('deliveryAddress'),
                    ewalletName: formData.get('ewalletName'),
                    ewalletAccount: ewalletAccount ? `+639${ewalletAccount}` : null
                  };
                  ...
                  {(selectedReward as any).category === 'E-wallet' ? (
                    <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        ...
                        />
                        <div className="relative">
                          <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1 text-gray-300 text-sm pointer-events-none select-none">
                          ...
                          <input 
                            ...
                            className="w-full pl-20 pr-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-orange-500 text-sm" 
                            maxLength={12}
                            ...
                            onInput={(e: React.FormEvent<HTMLInputElement>) => {
                              ...
                              if (value.length > 5) {
                                e.currentTarget.value = `${value.slice(0, 2)} ${value.slice(2, 5)} ${value.slice(5)}`
                              ...
                              if (digitCount !== 9 && digitCount > 0) {
                                e.currentTarget.setCustomValidity('Please enter exactly 9 digits')
                              } else {
                    ...
                    <>
                      ...
                      <input 
                        ...
                        name="email"
                        placeholder="Email Address" 
                        className="w-full px-3 py-2 bg-gray-700/50 border rounded-lg text-white placeholder-gray-400 focus:outline-none text-sm" 
                        onInput={(e: React.FormEvent<HTMLInputElement>) => {
                          const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
                          const isValid = emailRegex.test(e.currentTarget.value)
                          e.currentTarget.className = `w-full px-3 py-2 bg-gray-700/50 border rounded-lg text-white placeholder-gray-400 focus:outline-none text-sm ${isValid || e.currentTarget.value === '' ? 'border-gray-600 focus:border-orange-500' : 'border-red-500 focus:border-red-500'}`
                        }}
                        required 
                      />
                      <div className="relative">
